...
data List a = Node
 ...
 | DelNode
    {del_next :: (RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a))}
 | Null
...
{-@ data ListHandle a = ListHandle
  {lh_head :: IORef (RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a))
...
{-# INLINE myNext #-}
{-@ myNext :: l:List a ->
  { r:RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a) | (nxt l) = r }
@-}
...
{-@ measure isHead :: List a -> Bool
    isHead (Head n) = true
@-}
{-@ measure isNode :: List a -> Bool
    isNode (Node v n) = true
@-}
{-@ measure val :: List a -> a
    val (Node v n) = v
@-}
{-@ measure isDel :: List a -> Bool
...
find (ListHandle head _) x =
  do startPtr <- readIORef head
     gon startPtr
    where
      {-@ gon :: RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a) -> IO Bool @-}
      gon !prevPtr =
...
--                                 {b:Bool | (((terminalValue r) = v) <=> (pastValue r v))}
                        ...
                        case prevNode of
                          ...
                          Node prevVal _ -> do b <- rgListCAS prevPtr prevNode (Node prevVal (liquidAssume (axiom_pastIsTerminal curPtr curNode (terminal_listrg curPtr curNode) (terminal_listrg curPtr curNode)) nextNode))
                                               if b then gon prevPtr else gon curPtr
                          ...
                          Head _ -> do b <- rgListCAS prevPtr prevNode (Head (liquidAssume (axiom_pastIsTerminal curPtr curNode (terminal_listrg curPtr curNode) (terminal_listrg curPtr curNode)) nextNode))
                                       if b then gon prevPtr else gon curPtr
                          DelNode _ -> gon curPtr    -- if parent deleted simply move ahead
                          _ -> error "Impossible"
                _ -> error "Impossible"
              {-
...
iterateList itPtrPtr =
  do startPtr <- readIORef itPtrPtr
     gob startPtr
   where
      {-@ gob :: RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a) -> IO (Maybe (RGRef<{\x -> (1 > 0)},{\x y -> (ListRG x y)},{\x y -> (ListRG x y)}> (List a))) @-}
      gob prevPtr =
        do do prevNode <- forgetIOTriple (readRGRef prevPtr)
              let curPtr = myNext prevNode -- head/node/delnode have all next
              curNode <- forgetIOTriple (readRGRef curPtr)
              case curNode of
                ...
                DelNode nextNode ->
                        ...
                        case prevNode of
                          Node v _ -> do b <- rgListCAS prevPtr prevNode (Node v (liquidAssume (axiom_pastIsTerminal curPtr curNode (terminal_listrg curPtr curNode) (terminal_listrg curPtr curNode)) nextNode))
                                         if b then gob prevPtr else gob curPtr
                          --Head _ -> do b <- rgListCAS prevPtr prevNode (Head nextNode)
                          Head _ -> do b <- rgListCAS prevPtr prevNode (Head (liquidAssume (axiom_pastIsTerminal curPtr curNode (terminal_listrg curPtr curNode) (terminal_listrg curPtr curNode)) nextNode))
                                       if b then gob prevPtr else gob curPtr
                          DelNode _ -> gob curPtr    -- if parent deleted simply move ahead
                          _ -> error "Impossible"
                _ -> error "Impossible"
--
