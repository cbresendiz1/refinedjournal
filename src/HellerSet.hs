...
--  | DelNode {del_val  :: a<p>, slack :: { v : a | del_val  <= v}, del_next  :: (RGRef<{\x -> (1 > 0)},{\x y -> (SetRG x y)},{\x y -> (SetRG x y)}> (Set <{\x -> (slackD < x)}> a))}
--  | Null
--  | Head {head_next :: (RGRef<{\x -> (1 > 0)},{\x y -> (SetRG x y)},{\x y -> (SetRG x y)}> (Set <{\x -> (1 > 0)}> a))}
-- @-}
data Set a = Node {node_val  :: a
                  ,slack     :: a
                  ,node_next :: (UNPACK(RGRef (Set a)))}
            | DelNode {del_val  :: a
                      ,slack    :: a
                      ,del_next :: (UNPACK(RGRef (Set a)))}
            | Null
            | Head {head_next :: (UNPACK(RGRef (Set a)))}
           deriving Eq
...
{-# INLINE myNext #-}
--{-@ myNext :: l:Set a ->
--   {r:RGRef<{\x -> (1 > 0)},{\x y -> (SetRG x y)},{\x y -> (SetRG x y)}> (Set <{\x -> ((IsHead l) || (slack l < x))}> a) | ((nxt l) = r) } @-}
...
--{-@ assume injectStableSet :: forall <p :: (Set a) -> Bool,
--                                   q :: (Set a) -> Bool,
--                                   r :: (Set a) -> (Set a) -> Bool,
--                                   g :: (Set a) -> (Set a) -> Bool,
--                                   z :: a -> Bool>.
...
-- {-@ assume downcast_set :: forall <p :: a -> Bool>.
--                     { x::a |- { v:a | x < v } <: a<p> }
...
-- {-@ assume downcast_set_null :: forall <p :: a -> Bool>.
--                     { x::a |- Set <{\v -> x < v}> a <: Set <p> a }
--                     ref:RGRef<{\x -> (1 > 0)},{\x y -> (SetRG x y)},{\x y -> (SetRG x y)}> (Set <p> a) ->
--                     x:a ->
...
--
-- {-@ valnode_stable :: forall <q :: a -> Bool>.
--                       x:a ->
